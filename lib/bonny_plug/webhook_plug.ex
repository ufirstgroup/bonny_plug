defmodule BonnyPlug.WebhookPlug do
  ...
  In a Phoenix router you would forward post requests to this plug:
    ...
    post "/admission-review/validate", BonnyPlug.WebhookPlug, [webhook_type: :validating_webhook]
    post "/admission-review/mutate", BonnyPlug.WebhookPlug, [webhook_type: :mutating_webhook]

  """

  import Plug.Conn

  alias BonnyPlug.{AdmissionReview, WebhookHandler}
  ...
  def call(%Plug.Conn{method: "POST"} = conn, opts) do
    ...
    case process(conn.body_params, webhook_type) do
      {:ok, response_body} ->
        conn
  ...
  @spec process(map(), atom()) :: {:ok, binary()} | {:error, integer(), binary()}
  def process(%{"apiVersion" => @api_version, "kind" => @kind, "request" => request}, webhook_type) do
    response = request
               |> AdmissionReview.create()
               |> WebhookHandler.handle_webhook(webhook_type)
               |> Map.get(:response)

    response_body = Jason.encode!(%{"apiVersion" => @api_version, "kind" => @kind, "response" => response})
    {:ok, response_body}
  end
  def process(_, _), do: {:error, 400, "Unsupported Payload. This service expects a k8s admission review json document of apiVersion \"admission.k8s.io/v1\"."}

defmodule BonnyPlug.WebhookHandler do
  ...
  * `api_versions` - A list of versions of the resource, e.g. `["v1beta1", "v1"]`
  ...
  defmacro __using__(opts) do
    %{group: group, plural: plural, api_versions: api_versions} =
      case Map.new(opts) do
        %{crd: crd} ->
          read_crd(crd)

        %{group: _, plural: _, api_versions: _} = bindings ->
          bindings

        _ ->
          raise(
            ArgumentError,
            "Wrong usage of BonnyPlug.WebhookHandler. You have to pass either `crd: \"path-to-crd.yaml\"` or all three of `group`, `plural` and `api_verions` when using BonnyPlug.WebhookHandler"
          )
      end
  ...
  defp read_crd(path_to_crd) do
    crd =
      case YamlElixir.read_from_file(path_to_crd) do
        {:ok, crd} ->
          crd
        ...
        {:error, %YamlElixir.FileNotFoundError{message: error}} ->
          raise(
            ArgumentError,
            "Wrong usage of BonnyPlug.WebhookHandler. The CRD you passed was not found: " <> error
          )

        {:error, %YamlElixir.ParsingError{message: error}} ->
          raise(
            ArgumentError,
            "Wrong usage of BonnyPlug.WebhookHandler. The CRD YAML file you passed could not be parsed: " <>
              error
          )
      end
  ...
  defp derive_api_versions(_),
    do:
      raise(
        ArgumentError,
        "CRD version not supported. Currently only CRD versions v1 and v1beta1 are supported."
      )
end
